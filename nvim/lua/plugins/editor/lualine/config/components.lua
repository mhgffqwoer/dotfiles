...
local palette = require("plugins.editor.lualine.config.palette")
...
function cpn.branch(sep_type)
	return {
		...
		fmt = function(text)
			...
			return require("tools.lualine").build_component(config, {
...
function cpn.diff(sep_type)
	...
	local added_hl_gr = require("tools.lualine").get_hl_gr("Added")
	local modified_hl_gr = require("tools.lualine").get_hl_gr("Modified")
	local removed_hl_gr = require("tools.lualine").get_hl_gr("Removed")
	return {
		"diff",
		colored = true,
		diff_color = {
		...
		fmt = function(text)
			...
			require("tools.once").once(function()
				local bg = require("tools.lualine").get_component_bg(sep_type)
		...
		cond = require("tools.lualine").hide_width,
	}
end
...
function cpn.filetype(sep_type)
	return {
		...
		fmt = function(text)
			...
			ft_text = require("tools.string").capitalize(ft_text)
			return require("tools.lualine").build_component(config, {
			...
			for _, source in ipairs(sources) do
				...
				if s then
					local colors = {
						...
						pending = require("tools.theme").highlight("Whitespace").fg,
						error = require("tools.theme").highlight("DiagnosticError").fg,
					}
					table.insert(texts, { text = Icons.brain[source], color = colors[s] })
				end
			end
			return require("tools.lualine").build_component(config, texts, sep_type or "fill", nil, " ")
		end,
		cond = function()
			return require("tools.table").any(sources, status)
		end,
	}
end
